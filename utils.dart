  ...
  void createHumans(String selectedDifficulty){
    ...
    while(counter < 100000){
      if(counter<populationPyramide[0]*1000){
        int min = 0;
        int max = 14;
        humans.add(new Human("Young"+counter.toString(),counter.toString(),min + rnd.nextInt(max - min)));
      }
      else if(counter<populationPyramide[1]*1000){
        int min = 15;
        int max = 64;
        humans.add(new Human("Middle"+counter.toString(),counter.toString(),min + rnd.nextInt(max - min)));
      }
      else {
        int min = 65;
        int max = 100;
        humans.add(new Human("Old"+counter.toString(),counter.toString(),min + rnd.nextInt(max - min)));
      }
      counter++;
    }
    /*for(int i = 0; i<100;i++){
